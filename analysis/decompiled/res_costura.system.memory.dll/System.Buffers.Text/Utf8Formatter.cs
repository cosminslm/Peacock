...
public static class Utf8Formatter
{
	...
	{
		char symbolOrDefault = System.Buffers.Text.FormattingHelpers.GetSymbolOrDefault(in format, 'G');
		if (value)
		{
	...
	{
		return System.Buffers.Text.FormattingHelpers.GetSymbolOrDefault(in format, 'G') switch
		{
	...
	public static bool TryFormat(decimal value, Span<byte> destination, out int bytesWritten, StandardFormat format = default(StandardFormat))
	{
		...
		case 'g':
		{
			...
			{
				throw new NotSupportedException(System.SR.Argument_GWithPrecisionNotSupported);
			}
	...
	{
		int num;
		switch (System.Buffers.Text.FormattingHelpers.GetSymbolOrDefault(in format, 'D'))
		{
		case 'D':
			num = -2147483612;
			break;
	...
	{
		char c = System.Buffers.Text.FormattingHelpers.GetSymbolOrDefault(in format, 'c');
		switch (c)
		{
		case 'T':
		case 't':
