...
internal static class SpanHelpers
{
	...
	public static class PerTypeValues<T>
	{
		...
		{
			T[] array = new T[1];
			return Unsafe.ByteOffset(in Unsafe.As<Pinnable<T>>(array).Data, in array[0]);
		}
	}
	...
	public unsafe static int IndexOf(ref byte searchSpace, byte value, int length)
	{
		...
		{
			int num = (int)Unsafe.AsPointer(in searchSpace) & (Vector<byte>.Count - 1);
		...
		while (true)
		{
			...
			if (Vector.IsHardwareAccelerated && (int)(void*)intPtr < length)
			{
				...
				{
					Vector<byte> vector2 = Vector.Equals(vector, Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref searchSpace, intPtr)));
	...
	public unsafe static int LastIndexOf(ref byte searchSpace, byte value, int length)
	{
		...
		while (true)
		{
			...
			if (Vector.IsHardwareAccelerated && (void*)intPtr != null)
			{
				...
				{
					Vector<byte> vector2 = Vector.Equals(vector, Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref searchSpace, intPtr - Vector<byte>.Count)));
					if (Vector<byte>.Zero.Equals(vector2))
					{
	...
	public unsafe static int IndexOfAny(ref byte searchSpace, byte value0, byte value1, int length)
	{
		...
		while (true)
		{
			...
			if (Vector.IsHardwareAccelerated && (int)(void*)intPtr < length)
			{
				...
				for (; (void*)intPtr2 > (void*)intPtr; intPtr += Vector<byte>.Count)
				{
					Vector<byte> left = Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref searchSpace, intPtr));
	...
	public unsafe static int LastIndexOfAny(ref byte searchSpace, byte value0, byte value1, int length)
	{
		...
		while (true)
		{
			...
			if (Vector.IsHardwareAccelerated && (void*)intPtr != null)
			{
				...
				for (; (nuint)(void*)intPtr2 > (nuint)(Vector<byte>.Count - 1); intPtr2 -= Vector<byte>.Count)
				{
					Vector<byte> left = Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref searchSpace, intPtr - Vector<byte>.Count));
					Vector<byte> vector3 = Vector.BitwiseOr(Vector.Equals(left, vector), Vector.Equals(left, vector2));
					if (Vector<byte>.Zero.Equals(vector3))
					{
						intPtr -= Vector<byte>.Count;
	...
	public unsafe static bool SequenceEqual(ref byte first, ref byte second, NUInt length)
	{
		...
		if (Vector.IsHardwareAccelerated && (nuint)(void*)intPtr2 >= (nuint)Vector<byte>.Count)
		{
			...
			while (true)
			{
				...
				{
					if (Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref first, intPtr)) != Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref second, intPtr)))
					{
						break;
					}
					intPtr += Vector<byte>.Count;
				...
				return Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref first, intPtr2)) == Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref second, intPtr2));
			}
		}
		else
		{
			...
			while (true)
			{
				if ((void*)intPtr2 > (void*)intPtr)
				{
					if (Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.AddByteOffset(ref first, intPtr)) != Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.AddByteOffset(ref second, intPtr)))
					{
						break;
					}
				...
				return Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.AddByteOffset(ref first, intPtr2)) == Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.AddByteOffset(ref second, intPtr2));
			}
		}
		goto IL_013f;
		IL_013f:
	...
	public unsafe static int SequenceCompareTo(ref byte first, int firstLength, ref byte second, int secondLength)
	{
		if (!Unsafe.AreSame(in first, in second))
		{
			...
			if (Vector.IsHardwareAccelerated && (nuint)(void*)intPtr3 > (nuint)Vector<byte>.Count)
			{
				...
				for (; (void*)intPtr3 > (void*)intPtr2 && !(Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref first, intPtr2)) != Unsafe.ReadUnaligned<Vector<byte>>(in Unsafe.AddByteOffset(ref second, intPtr2))); intPtr2 += Vector<byte>.Count)
				{
				}
			}
			else if ((nuint)(void*)intPtr3 > (nuint)sizeof(UIntPtr))
			{
				...
				for (; (void*)intPtr3 > (void*)intPtr2 && !(Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.AddByteOffset(ref first, intPtr2)) != Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.AddByteOffset(ref second, intPtr2))); intPtr2 += sizeof(UIntPtr))
				{
				}
			}
		...
		if (!Unsafe.AreSame(in first, in second))
		{
			...
			if ((nuint)(void*)intPtr >= (nuint)(sizeof(UIntPtr) / 2))
			{
				if (Vector.IsHardwareAccelerated && (nuint)(void*)intPtr >= (nuint)Vector<ushort>.Count)
				{
					...
					while (!(Unsafe.ReadUnaligned<Vector<ushort>>(in Unsafe.As<char, byte>(ref Unsafe.Add(ref first, intPtr2))) != Unsafe.ReadUnaligned<Vector<ushort>>(in Unsafe.As<char, byte>(ref Unsafe.Add(ref second, intPtr2)))))
					{
				...
				for (; (void*)intPtr >= (void*)(intPtr2 + sizeof(UIntPtr) / 2) && !(Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.As<char, byte>(ref Unsafe.Add(ref first, intPtr2))) != Unsafe.ReadUnaligned<UIntPtr>(in Unsafe.As<char, byte>(ref Unsafe.Add(ref second, intPtr2)))); intPtr2 += sizeof(UIntPtr) / 2)
				{
				}
			}
			if (sizeof(UIntPtr) > 4 && (void*)intPtr >= (void*)(intPtr2 + 2) && Unsafe.ReadUnaligned<int>(in Unsafe.As<char, byte>(ref Unsafe.Add(ref first, intPtr2))) == Unsafe.ReadUnaligned<int>(in Unsafe.As<char, byte>(ref Unsafe.Add(ref second, intPtr2))))
			{
				intPtr2 += 2;
			}
	...
	{
		IntPtr intPtr = Unsafe.ByteOffset(in src, in Unsafe.Add(ref src, srcLength));
		IntPtr intPtr2 = Unsafe.ByteOffset(in dst, in Unsafe.Add(ref dst, dstLength));
		IntPtr intPtr3 = Unsafe.ByteOffset(in src, in dst);
		bool num;
		if (sizeof(IntPtr) != 4)
		{
		...
		if (!num && !IsReferenceOrContainsReferences<T>())
		{
			...
			for (ulong num3 = 0uL; num3 < num2; num3 += num4)
			{
				...
				Unsafe.CopyBlock(ref Unsafe.Add(ref source, (IntPtr)(long)num3), in Unsafe.Add(ref source2, (IntPtr)(long)num3), num4);
			}
			return;
		}
		goto IL_00de;
	...
	public static int LastIndexOf<T>(ref T searchSpace, T value, int length) where T : IEquatable<T>
	{
		while (true)
		{
			...
			else
			{
				...
				while (!reference13.Equals(other13));
			}
			goto IL_02f3;
			IL_02f5:
			return length + 1;
