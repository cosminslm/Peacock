...
{
	private volatile object _stateUnion = null;

	private volatile AggregatorLookupFunc<TAggregator> _cachedLookupFunc = null;
	...
	{
		(_cachedLookupFunc = LabelInstructionCompiler.Create(ref this, _createAggregatorFunc, labels))(labels, out var aggregator);
		return aggregator;
	}
	...
	public void Collect(Action<LabeledAggregationStatistics> visitFunc)
	{
		...
		if (!(stateUnion is TAggregator val))
		{
			if (!(stateUnion is FixedSizeLabelNameDictionary<StringSequence1, ObjectSequence1, TAggregator> fixedSizeLabelNameDictionary))
			{
				if (!(stateUnion is FixedSizeLabelNameDictionary<StringSequence2, ObjectSequence2, TAggregator> fixedSizeLabelNameDictionary2))
				{
					if (!(stateUnion is FixedSizeLabelNameDictionary<StringSequence3, ObjectSequence3, TAggregator> fixedSizeLabelNameDictionary3))
					{
						if (!(stateUnion is FixedSizeLabelNameDictionary<StringSequenceMany, ObjectSequenceMany, TAggregator> fixedSizeLabelNameDictionary4))
						{
